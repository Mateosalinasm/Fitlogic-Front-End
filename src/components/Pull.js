import React from "react";
import { Link } from "react-router-dom";
import { motion } from "framer-motion";

const PullWorkout = () => {
  return (
    <div>
      <div className="push-parent-container">
        <div className="back-btn">
                <motion.button
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.9 }}
                    className="menu-btn"
                    onClick={() => null}
                > 
                    <Link to='/workout' onClick={() => window.scrollTo(0, 0)}>
                        <img className="back" src="/assets/images/icons/back-arrow.png" alt="back arrow" />
                    </Link>
                </motion.button>
            </div>
        <div className="push-container">
          <div className="push-title pull-title-center">
            <h1>Pull</h1>
          </div>
          <section className="slide-show">
            <img src="/assets/images/workout/back1.png" alt="" />
            <img src="/assets/images/workout/back2.png" alt="" />
            <img src="/assets/images/workout/bicep-curl-bg.png" alt="" />
          </section>
          <ul className="slide-show-links">
            <li>
              <Link to="/workout/pull/latissimus-dorsi">Latissimus Dorsi</Link>
            </li>
            <li>
              <Link to="/workout/pull/trapezius">Trapezius</Link>
            </li>
            <li>
              <Link to="/workout/pull/biceps">Biceps</Link>
            </li>
          </ul>
        </div>
      </div>
    </div>
  );
};

export default PullWorkout;
